...
return <AppContext.Provider value={{materials,sessions,errors,vocab,tasks,dictations,saveMaterial:(x,id)=>save(setMaterials,x,id),saveSession:(x,id)=>{save(setSessions,x,id);markActivity(x.date)},saveError:(x,id)=>save(setErrors,x,id),saveVocab:(x,id)=>{save(setVocab,x,id);markActivity()},saveTask:(x,id)=>{save(setTasks,x,id);if(x.done)markActivity()},saveDictation:(x,id)=>{save(setDictations,x,id);markActivity()},remove},activityDates}}>{children}</AppContext.Provider>}
