...
type Ctx={materials:Material[];sessions:Session[];errors:ErrorItem[];vocab:VocabItem[];tasks:Task[];dictations:Dictation[];activityDates:string[];saveMaterial:(x:Omit<Material,"id">,id?:number)=>void;saveSession:(x:Omit<Session,"id">,id?:number)=>void;saveError:(x:Omit<ErrorItem,"id">,id?:number)=>void;saveVocab:(x:Omit<VocabItem,"id">,id?:number)=>void;saveTask:(x:Omit<Task,"id">,id?:number)=>void;saveDictation:(x:Omit<Dictation,"id">,id?:number)=>void;remove:(type:string,id:number)=>void};
const AppContext=createContext<Ctx|null>(null);
export function AppProvider({children}:{children:ReactNode}){const [materials,setMaterials]=useState(initialMaterials),[sessions,setSessions]=useState(initialSessions),[errors,setErrors]=useState(initialErrors),[vocab,setVocab]=useState(initialVocab),[tasks,setTasks]=useState(initialTasks),[dictations,setDictations]=useState(initialDictations),[activityDates,setActivityDates]=useState<string[]>([]);
useEffect(()=>{try{const raw=localStorage.getItem("ail-data");if(raw){const d=JSON.parse(raw);setMaterials(d.materials||initialMaterials);setSessions(d.sessions||initialSessions);setErrors(d.errors||initialErrors);setVocab(d.vocab||initialVocab);setTasks(d.tasks||initialTasks);setDictations(d.dictations||initialDictations);setActivityDates(d.activityDates||[])}}catch{}},[]);
useEffect(()=>{try{localStorage.setItem("ail-data",JSON.stringify({materials,sessions,errors,vocab,tasks,dictations,activityDates}))}catch{}},[materials,sessions,errors,vocab,tasks,dictations,activityDates]);
const save=(setter:any,x:any,id?:number)=>setter((old:any[])=>id?old.map(v=>v.id===id?{...x,id}:v):[{...x,id:Date.now()},...old]);
const today=()=>new Date().toISOString().slice(0,10);const markActivity=(date?:string)=>setActivityDates(old=>Array.from(new Set([...old,date||today()])));const remove=(type:string,id:number)=>({materials:setMaterials,sessions:setSessions,errors:setErrors,vocab:setVocab,tasks:setTasks,dictations:setDictations} as any)[type]((old:any[])=>old.filter(v=>v.id!==id));
return <AppContext.Provider value={{materials,sessions,errors,vocab,tasks,dictations,activityDates,saveMaterial:(x,id)=>save(setMaterials,x,id),saveSession:(x,id)=>{save(setSessions,x,id);markActivity(x.date)},saveError:(x,id)=>save(setErrors,x,id),saveVocab:(x,id)=>{save(setVocab,x,id);markActivity()},saveTask:(x,id)=>{save(setTasks,x,id);if(x.done)markActivity()},saveDictation:(x,id)=>{save(setDictations,x,id);markActivity()},remove}}>{children}</AppContext.Provider>}
